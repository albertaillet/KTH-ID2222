...
class A_Priori:
    ...
    def count(baskets: list[set[int]], k: int, c_k: Optional[set[frozenset[int]]]=None) -> dict[frozenset[int], int]:
        '''Counts the number of occurances of each set'''
    ...
    @staticmethod
    def filter(counts: dict[frozenset[int], int], threshold: float) -> set[frozenset[int]]:
        return {i for i, count in counts.items() if count >= threshold}
    ...
    @staticmethod
    def generate(l_k_prev: set[frozenset[int]], l_1: set[frozenset[int]]) -> set[frozenset[int]]:
